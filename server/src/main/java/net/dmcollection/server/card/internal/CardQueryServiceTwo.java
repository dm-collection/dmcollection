...
@Service
public class CardQueryServiceTwo {
  private static final Logger log = LoggerFactory.getLogger(CardQueryServiceTwo.class);
  ...
  public record Printing(
      ...
      int amount,
      List<PrintingSide> sides) {}

  public record Card(int id, String name, List<Printing> printings) {}

  public record CardPage(List<Card> cards, int cardCount) {}

  private static final Field<Integer> AMOUNT_FIELD =
      coalesce(COLLECTION_ENTRY.QUANTITY, 0).as("amount");

  public CardPage search(@NonNull SearchFilter filter) {
    if (filter.isInvalid()) {
      log.warn("Invalid search filter: {}", filter);
      return new CardPage(Collections.emptyList(), 0);
    }
    ...
    var printingSides =
        multiset(
            ...
            .as("printingSides")
            .convertFrom(r -> r.map(s -> new PrintingSide(s.get(PRINTING_SIDE.IMAGE_FILENAME))));
    var printings =
        multiset(
                select(
                        PRINTING.ID,
    ...
    List<Card> cards = rows.map(r -> new Card(r.get(CARD.ID), r.get(CARD.NAME), r.get(printings)));
